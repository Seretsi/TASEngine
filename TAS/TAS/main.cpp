// Todo: Add code to optionally protect aspect ratios
...
struct UniformBufferObject {
	// alignas helps maintain bit alignment
...
private:
	...
	// looking for appropriate formats (best available from input ordered list)
	...
	void createTextureSampler() {
		...
		samplerInfo.maxAnisotropy = 16.0f; // Max on known HW today
	...
	VkImageView createImageView(VkImage& image, uint32_t mipLevel, VkFormat format, VkImageAspectFlags aspectFlags) {
	...
	void transitionImageLayout(VkImage image, uint32_t mipLevel, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout) {
	...
	void createTextureImage() {
		...
		transitionImageLayout(textureImage, mipLevels, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
	...
	void updateUniformBuffer(uint32_t currentImage) {
		...
		ubo.proj[1][1] *= -1; // glm is made for OpenGL and we need to flip the y-axis for vulkan. Here we flip the sign of projection matrix's scaling factor
	...
	void createCommandBuffers() {
		...
		for (size_t i = 0; i < commandBuffers.size(); i++) {
			...
			// starting adding commands in queue to be executed
	...
	void createGraphicsPipeline() {
		...
		// need to change pipeline values mentioned in the struct at draw time
	...
	VkSurfaceFormatKHR chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
		...
		//return the first one we have which will likely be just fine, enough.
	...
	QueueFamilyIndices findQueueFamilies(VkPhysicalDevice device) {
		...
		// remember the last family we find for each requirement
		// TODO: logic to pick one family to handle both would improve performance.
		int i = 0;
	...
	*/
	void cleanup() {
		...
		vkDestroyBuffer(device, vertexBuffer, nullptr);
		vkFreeMemory(device, vertexBufferMemory, nullptr); //Todo: memory bug here when closing window
